...
def material(name, color, roughness=.75, metal=0):
    color=tuple(linear(v) for v in color)
...
def loft(parent,mat,rings,n=12):
    ...
    for poly in mesh.polygons:
        coords=[((mesh.loops[i].vertex_index % n)/n, (mesh.loops[i].vertex_index//n)/(len(rings)-1)) for i in poly.loop_indices]
        seam=max(c[0] for c in coords)-min(c[0] for c in coords)>.5
        for i,(u,v) in zip(poly.loop_indices,coords):uv.data[i].uv=(1 if seam and u==0 else u,v)
...
for idx,(name,col,dark,skin,hair) in enumerate(COLORS):
    ...
    mats={'cloth':material(name+'_jacket',col,.85),'pants':material(name+'_trousers',dark,.88),'skin':material(name+'_skin',skin,.56),
    ...
    bar(parts['torso'],iv,(-.23,.85,.22),(.12,.36,.25),.026)
    bar(parts['torso'],iv,(.25,.84,.23),(-.20,.13,.25),.03)
    bar(parts['torso'],gold,(.265,.84,.24),(-.185,.13,.265),.009)
    for t in range(11):
        q=t/10;bar(parts['torso'],iv,(.24-.42*q,.82-.66*q,.27),(.255-.42*q,.81-.66*q,.27),.006)
    loft(parts['torso'],cloth,[(.84,.195,.158,.03),(.91,.183,.157,.03),(1.01,.14,.14,.03)],16)
    ...
    h=parts['head'];loft(h,sk,[(-.13,.105,.105,.04),(-.035,.16,.155,.055),(.1,.20,.19,.025),(.27,.22,.20,0),(.42,.185,.17,-.01),(.48,.07,.07,-.025)],24)
    ellipsoid(h,sk,(-.035,-.20,0),(.115,.18,.12),12,8)
    ellipsoid(h,sk,(-.18,.17,.14),(.047,.073,.052))
    ellipsoid(h,sk,(.22,.22,.07),(.024,.085,.031),16,10)
    ellipsoid(h,sk,(.255,.15,.075),(.041,.035,.042),16,10)
    ellipsoid(h,sk,(.14,-.019,.063),(.095,.064,.122),16,10)
    ...
    for i in range(9):
        a=i*2.399
        lock=empty(name+'_hairLock',h);lock.location=xyz((-.02+math.cos(a)*.15,.35,math.sin(a)*.18))
        loft(lock,mats['hair'],[(0,.09,.08,0),(.12,.10,.075,-.025),(.24,.055,.04,-.085),(.31+(i%3)*.03,.006,.005,-.17)],8)
        for o in list(lock.children):
            o.location+=lock.location;o.parent=h
    ...
    for side in ['F','B']:
        ...
        loft(p,gold,[(.291,.208,.191,0),(.304,.204,.189,0)])
        ...
        p=parts['foot'+side];box(p,sole,(.13,.06,.0),(.56,.10,.29),.035)
        box(p,pants,(.13,.17,0),(.51,.19,.275),.06)
        bar(p,iv,(-.06,.11,.149),(.32,.11,.149),.01)
    ...
    bpy.ops.export_scene.gltf(filepath=str(OUT/(name+'-rig-v3.glb')),export_format='GLB',use_selection=True,export_animations=False,export_extras=True)
...
# Keep the editable Blender material preview in sync with the shared runtime map.
image=bpy.data.images.load(str(OUT/'wind-jacquard-v3.jpg'));image.pack()
for mat in bpy.data.materials:
    if not mat.name.endswith(('_jacket','_trousers')):continue
    nodes=mat.node_tree.nodes;links=mat.node_tree.links
    bs=nodes.get('Principled BSDF');color=bs.inputs['Base Color'].default_value[:]
    tex=nodes.new('ShaderNodeTexImage');tex.image=image
    mult=nodes.new('ShaderNodeMixRGB');mult.blend_type='MULTIPLY';mult.inputs[0].default_value=1;mult.inputs[2].default_value=color
    links.new(tex.outputs['Color'],mult.inputs[1]);links.new(mult.outputs[0],bs.inputs['Base Color'])
...
bpy.ops.wm.save_as_mainfile(filepath=str(Path(__file__).parent/'crosswind-fighters-v3.blend'))
print('FURY_ASSETS_DONE',[(p.name,p.stat().st_size) for p in OUT.glob('*-rig-v3.glb')])
