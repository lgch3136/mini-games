...
2026-09-05 重建；2026-09-06 修正关节、脚步与材质光照。本轮不修改跑酷、突击队或其他游戏。最新验收见 [20260906-polish.md](20260906-polish.md)。

## 制作与运行

- Blender 4.5.13 LTS：程序化创建三名原创角色的实体网格、服装、面部、肘膝关节和具名分段控制结构。最新源文件 `english-word-fury/art/crosswind-fighters-v3.blend` 可继续编辑；生成脚本为同目录 `build_fighters.py`。
- 导出三个 v3 GLB（约 371–405 KB / 人），运行时通过 Three.js WebGL 加载。模型不是渲染成图片再贴回画面，也不需要玩家安装 Blender。
- 这是**刚性分段关节模型**，不是一套蒙皮变形 / 动捕角色库。两段 IK 维持肢体长度，动作关键姿势由战斗帧表驱动；物理 60 Hz，显示位置与关节姿势插值。
- 固定 16:9、固定世界距离，手机只缩放视图，不改变移动速度、攻击范围或碰撞盒。开场有最多 5.5% 的缓慢拉远，交手前完全回到固定镜头；无屏幕震动或全屏命中闪光。系统减少动态效果设置会关闭开场镜头、漂浮尘粒及拳脚轨迹；菜单也可关闭氛围光效。
...
暂停、出招表、退出、失焦、后台隐藏均停止 RAF、音乐定时器并清理活动音源。页面离开时关闭 AudioContext、释放模型几何体 / 材质 / 共享纹理 / WebGL renderer。渲染 DPR 上限 1.5；凌风 + 赤绫约 20K 三角形，98 次 draw call；特效最多 36 项、局部光源池 2 个，每名角色轨迹最多 5 点。氛围底层缓存复用，不逐帧生成纹理。
